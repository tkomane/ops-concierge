...
- **MCP-inspired tool surface (conceptual):** `azure.query`, `onprem.query`, `session.ack`, `itsm.similar`, `calendar.propose`, `ticket.open` as visible timeline steps (not a live MCP server).
...
- Simulated path lets a solo SA architect demo hybrid Azure + on-prem ops without hardware.
...
- No public sample feeds for Azure Service Health + GreenLake-like events.
...
**Yes** — for hybrid-ops and incident concierge ideas that need session memory and multi-system tools. Prefer simulation or MCP server path over buying Bee/Ring hardware for a first hackathon entry.
