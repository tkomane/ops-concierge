...
const root = path.resolve(process.argv[2] || '/private/tmp/ops-concierge-review-003-corrections');
...
async function retry() {
  ...
  await c.handleApprove('approve',{intent:'approve'});
  return {method:'Production client, runner, approval coordinator and store; injected HTTP responses',first,afterRetry:{phase:c.state.phase,counts:c.store.getActionCounts(),calls,messages:c.state.messages,proposal:c.state.proposal,artifact:c.ticketText(c.state.scenario)}};
}
...
(async()=>{
  ...
  const out={checkout:root,conditionals,uncertainty,droppedResponse:await droppedResponse(),retry:await retry(),actualHttp:await actualHttp()};
